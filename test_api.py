...
-------------------------------------------
This script automatically tests the core endpoints of your Inventory Management API.

Prerequisites
-------------
Python 3.6+ and the `requests` library.
...
2. Set BASE_URL below to your server’s base URL.
3. Run:
    python test_api.py
"""
...
#  HELPER FUNCTION
# -------------------------------------------------------------------
def print_result(test_name, passed, expected=None, got=None,
                 request_data=None, response_body=None):
...
def test_register_user():
    ...
    print_result("User Registration", passed, "201 or 409", res.status_code,
                 payload, res.text)
...
def test_login():
    ...
    print_result(
        "Login Test",
        passed,
        expected={"username": payload["username"], "password": payload["password"]},
        got=res.text,
...
def test_add_product(token):
    """POST /products – adds a new product; returns product_id"""
    payload = {
        "name": "Phone",
        "type": "Electronics",
        "sku": "PHN-001",
    ...
    if passed:
        ...
        print_result("Add Product", False, 201, res.status_code,
                     payload, res.text)
        return None


def test_update_quantity(token, product_id, new_quantity):
    """PUT /products/{id}/quantity – updates quantity field"""
    ...
    if passed:
        ...
        print_result("Update Quantity", False, 200, res.status_code,
                     payload, res.text)


def test_get_products(token, expected_quantity):
    """GET /products – verifies the quantity of 'Phone'"""
    headers = {"Authorization": f"Bearer {token}"}
    res = requests.get(f"{BASE_URL}/products", headers=headers)

    if res.status_code != 200:
        print_result("Get Products", False, 200, res.status_code,
                     None, res.text)
        return
    ...
    try:
        ...
        print_result("Get Products", False, "valid JSON list",
                     "Invalid JSON", None, res.text)
        return
...
# -------------------------------------------------------------------
def run_all_tests():
    """Runs tests sequentially; aborts chain on critical failure."""
    test_register_user()
